...
2. Places UP/DOWN orders at 0.50.
3. Monitors fills -> places auto-sell at 0.55.
...
CONFIG_FILE = Path(__file__).parent / "bot_config_scalping.json"
...
logger = logging.getLogger("BTC_5Min_Scalping")
...
class Btc5MinScalpingStrategy(BaseStrategy):
    """
    BTC 5-Minute Scalping Strategy.
    Buys at 0.50, Sells at 0.55.
    """
    def __init__(self, bot: TradingBot, params: Optional[Dict[str, Any]] = None):
        super().__init__(bot, params or {}, name="Btc5MinScalping")
    ...
    def _load_config_value(self, key: str, default):
        """Load a single value from bot_config_scalping.json, falling back to default."""
        import json
        try:
    ...
    def _save_config(self) -> None:
        """Persist all runtime-adjustable settings to bot_config_scalping.json."""
        import json
        try:
            data = {}
            if CONFIG_FILE.exists():
...
async def main():
    ...
    strategy = Btc5MinScalpingStrategy(bot, params={"check_interval": 1})

    logger.info("Starting BTC 5Min Scalping Strategy...")
